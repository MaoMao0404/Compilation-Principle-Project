...
string nfadfa::regexp_to_postfix(string regexp)
{
    ...
    for(unsigned int i=0; i<regexp.size(); i++)
    {
//        ((s[i]>=47 && s[i]<=90)||(s[i]>=95 && s[i]<=122)||s[i]==123||s[i]==125||s[i]==33)
        if((regexp[i]>=47 &&regexp[i]<=90)||(regexp[i]>=95 && regexp[i]<=122)||regexp[i]==123||regexp[i]==125||regexp[i]==33||regexp[i]==44)
            postfix+=regexp[i];
        else
             switch(regexp[i])
            {
...
void nfadfa::postfix_to_nfa(string postfix)
{
    ...
    {
//        ((s[i]>=47 && s[i]<=90)||(s[i]>=95 && s[i]<=122)||s[i]==123||s[i]==125||s[i]==33)
        if((postfix[i]>=47 && postfix[i]<=90)||(postfix[i]>=95 && postfix[i]<=122)||postfix[i]==123||postfix[i]==125||postfix[i]==33||postfix[i]==44)
        {
            character(postfix[i]);
            NumOfChar++;
...
void nfadfa::getCcode(int v, vector<string> &lines)
{
    ...
    {
        //生成的对应C语言分析程序，首句为“char ch=in.get()”
        lines.push_back("char ch = in.get();");
        string line="while(";
        int i=0;
...
void nfadfa::show_code(string zzz)
{
    vector<string> lines;
    lines.push_back("void Code(string INPUT_PATH) {");
    lines.push_back("ifstream in;");
    lines.push_back("in.open(INPUT_PATH);");
    lines.push_back("string s;");
    lines.push_back("int k = 1;");
...
//    ofstream outfile("compress.h",ios::app);
//    for(int i=0;i<lines.size();i++)
//    {
